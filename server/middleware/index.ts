...
import { movieFileRead } from '../service/movie.service'
...
export const handleValidationErrors = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!errors.isEmpty()) {
        return res.status(400).json({ message: t('message', 'INVALID_EMAIL') })
    }
    next()
}
...
export const authenticateToken = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) {
        return res.status(401).json({ message: t('message', 'ACCESS_DENIED') })
    }
...
export const isMovieAvailable = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!id) {
        return res.status(401).json({ message: t('message', 'PROVIDE_MOVIE_ID') })
    }
    const rawData: any = await movieFileRead()

    if (rawData.message !== "Success")return res.status(401).json({ message: t('message', 'MOVIE_NOT_FOUND') })
        const movieData = JSON.parse(rawData?.movieList)
    const newJson = movieData.filter((a: any) => a.id === id)
    if (newJson.length === 0) return res.status(401).json({ message: t('message', 'MOVIE_NOT_FOUND') })
    next()
}
